					...
					while ($query->have_posts()) : $query->the_post(); 
						...
						<div class="profile-info-widget">
							...
							<div class="appointment-action">
								...
								<a href="javascript:;" class="dc-btn dc-btn-sm btn btn-sm bg-info-light mb-2 mt-0" id="dc-booking-service" data-type="doctor" data-id="<?php echo intval($post->ID); ?>"><i class="far fa-eye"></i><?php esc_html_e('View','doccure');?></a>
								<?php 
						      	$post_status_reschedule	= get_post_status( $post->ID ); 
 								if($post_status_reschedule == 'publish'){ ?>
								<a href="javascript:;" class="dc-btn dc-btn-sm btn btn-sm dc-add-booking-reschedule bg-info-sucess" data-postid="<?php echo intval($post->ID); ?>"><i class="far fa-calendar"></i><?php esc_html_e('Reschedule','doctreat');?></a>
								<?php } ?>
