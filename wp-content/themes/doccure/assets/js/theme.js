(function ($) {
    ...
    if ($('.header-layout-10').length > 0) {
		$(document).ready(function () {
			$(window).scroll(function () {
				var scroll = $(window).scrollTop();
				if (scroll > 35) {
					$(".header-layout-10").addClass('header-sticky');
				}

				else {
					$(".header-layout-10").removeClass('header-sticky');
				}
			})
		})
	}
...
function printSection(button) {
    ...
    printWindow.document.write('<style>@media print { .print-button { display: none; } table { width: 100%; border-collapse: collapse; } th, td { border: 1px solid #ddd; padding: 8px; } th { background-color: #f2f2f2; } }</style>');
